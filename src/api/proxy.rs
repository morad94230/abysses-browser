...
async fn handle_request(req: Request<Body>) -> Result<Response<Body>, Infallible> {
    ...
    if host.ends_with(".abyss") {
        ...
        Ok(Response::builder().status(200).header("content-type", "text/html").body(Body::from(html)).unwrap())
    } else if req.uri().path() == "/health" {
        Ok(Response::builder().status(200).body(Body::from("OK")).unwrap())
    } else {
        Ok(Response::builder().status(403).body(Body::from("Abysses only serves .abyss domains")).unwrap())
    }
}