...
impl NetworkManager {
    pub async fn new(keypair: Libp2pKeypair, listen_addresses: Vec<String>) -> Result<Self, AbyssError> {
        ...
        let noise_transport = noise::NoiseAuthenticated::new(quic_transport, noise_config, &keypair)
            .map_err(|e| AbyssError::Network(format!("Noise: {}", e)))?;
        let behaviour = AbyssBehaviour {
            kademlia: Kademlia::with_config(peer_id, MemoryStore::new(peer_id),
            ...
            identify: libp2p::identify::Behaviour::new(
                libp2p::identify::Config::new("/abyss/1.0.0".to_string(), keypair.public())),
        };
        let swarm = SwarmBuilder::with_tokio_executor(noise_transport, behaviour, peer_id).build();
        let mut nm = Self { swarm, local_peer_id: peer_id, keypair };
        for addr in listen_addresses {
            nm.swarm.listen_on(addr.parse().map_err(|e| AbyssError::Network(format!("Addr: {}", e)))?)
                .map_err(|e| AbyssError::Network(format!("Listen: {}", e)))?;
        }
        Ok(nm)
    }

    pub fn local_peer_id_string(&self) -> String { self.local_peer_id.to_string() }
    pub fn noise_public_key(&self) -> Vec<u8> { self.keypair.public().encode_protobuf() }
}