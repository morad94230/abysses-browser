...
impl ResilienceTest {
    ...
    pub fn run(&mut self, pct: f64, msgs: usize) -> ResilienceReport {
        ...
        for _ in 0..msgs / 2 {
            ...
            if a != b {
                before.push(self.network.send(a, b));
            }
        }
        ...
        for _ in 0..msgs / 2 {
            ...
            let b = &nodes[rand::random::<usize>() % nodes.len()];
            if a != b {
                after.push(self.network.send(a, b));
            }
        }
        let before_rate =
            before.iter().filter(|m| m.success).count() as f64 / before.len().max(1) as f64;
        let after_rate =
            after.iter().filter(|m| m.success).count() as f64 / after.len().max(1) as f64;
        ResilienceReport {
            before_rate,
            after_rate,
...
impl ResilienceReport {
    pub fn print(&self) {
        println!(
            "Nodes: {} total, {} killed",
            self.nodes_total, self.nodes_killed
        );
        println!(
            "Before: {:.1}% After: {:.1}%",
            self.before_rate * 100.0,
            self.after_rate * 100.0
        );
        println!(
            "Resilience: {}",
            if self.resilient { "PASS" } else { "FAIL" }
        );
    }
}