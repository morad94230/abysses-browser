...
use crate::circuit::builder::{CircuitBuilder, OnionCircuit};
...
impl CircuitManager {
    pub fn new(
        ...
        health_checker: CircuitHealthChecker,
        max_circuits: usize,
    ) -> Self {
        Self {
    ...
    fn should_rotate(&self, circuit: &OnionCircuit) -> bool {
        let now = SystemTime::now()
            ...
            .unwrap()
            .as_secs();
        circuit.message_count >= self.max_messages_per_circuit
            || now - circuit.created_at > self.max_circuit_lifetime_seconds
    }
    ...
    pub async fn get_or_create_circuit(&mut self) -> Result<String, AbyssError> {
        if let Some(ref id) = self.active_circuit_id {
            if let Some(c) = self.circuits.get(id) {
                ...
                } else if self.health_checker.check(c) == CircuitHealth::Healthy {
                    return Ok(id.clone());
                }
            }
        }
        for (id, c) in &self.circuits {
            if self.health_checker.check(c) == CircuitHealth::Healthy {
                self.active_circuit_id = Some(id.clone());
                return Ok(id.clone());
            }
        }
        if self.circuits.len() >= self.max_circuits {
            if let Some(oldest) = self.circuits.keys().next().cloned() {
    ...
    ) -> Result<Vec<u8>, AbyssError> {
        let circuit = self
            .circuits
            .get_mut(circuit_id)
            .ok_or(AbyssError::NoActiveCircuit)?;
