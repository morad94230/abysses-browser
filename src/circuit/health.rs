...
pub enum CircuitHealth { Healthy, Degraded, Failing, Dead }
...
impl CircuitHealthChecker {
    pub fn check(&self, circuit: &OnionCircuit) -> CircuitHealth {
        ...
        if circuit.expires_at < now { return CircuitHealth::Dead; }
        if now - circuit.last_used > 300 && circuit.message_count > 0 { return CircuitHealth::Degraded; }
