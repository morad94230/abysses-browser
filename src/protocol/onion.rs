...
fn generate_secure_nonce() -> [u8; 12] {
    ...
    for i in 0..12 { nonce[i] ^= random[i]; }
    nonce
}
...
pub enum FinalPayload {
    HttpRequest { method: String, path: String, headers: Vec<(String, String)>, body: Vec<u8>, target_hash: [u8; 32] },
    StoreRequest { fragment_hash: [u8; 32], fragment_data: Vec<u8>, ttl: u32 },
    RetrieveRequest { name: String, expected_hash: [u8; 32] },
    CoverTraffic { nonce: [u8; 32], padding: Vec<u8> },
}
...
pub enum FinalResponse {
    ...
    StoreAck { fragment_hash: [u8; 32], stored_by: Vec<String> },
    CoverAck { nonce: [u8; 32] },
}
