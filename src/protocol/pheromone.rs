...
impl PheromoneTable {
    pub fn add_neighbor(&mut self, peer_id: String) {
        if self.scores.len() >= self.max_neighbors {
            self.evict_weakest();
        }
        self.scores
            .entry(peer_id.clone())
            .or_insert_with(PheromoneScore::new);
    ...
    pub fn select_relay(&mut self, exclude: &[String]) -> Option<String> {
        let available: Vec<String> = self
            ...
            .filter(|k| {
                self.states
                    .get(*k)
                    .map_or(true, |s| s != &NeighborState::Blacklisted)
            })
            .cloned()
            .collect();
    ...
    pub fn deposit_failure(&mut self, relay: &str) {
        if let Some(s) = self.scores.get_mut(relay) {
            ...
            if s.failure_count >= self.blacklist_threshold as u64 {
                self.states
                    .insert(relay.to_string(), NeighborState::Blacklisted);
            }
        }
    }
